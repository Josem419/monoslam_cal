...
class LocalMapper:
        ...
        while not self.data_manager.shutdown:
            ...
            if self.data_manager.data.currentMap is not None:
                ...
                if len(self.data_manager.data.MKFs) == 0:
                    data["new_mappoints"] = (
                        self.data_manager.data.currentMap.mappoints
                    )  # initial points mapped in tracker
                else:
    ...
    def mapNewPoints(self, this_MKF: MultiFrame):
        ...
        for i in range(
            len(self.data_manager.data.MKFs) - 1,
            max(
                len(self.data_manager.data.MKFs)
                - (2 + self.settings["mapping_seach_n_frames"]),
                -1,
            ),
            -1,
        ):  # propagate backwards:
            ...
            for cam in range(self.data_manager.config.nrcams):
                ...
                matches = matcher.match(
                    other_MKF.descriptors[cam], this_MKF.descriptors[cam]
                )
                ...
                new_matches = [
                    ...
                    if not this_MKF.associations[cam][match.trainIdx]
                    and not other_MKF.associations[cam][match.queryIdx]
                ]

                new_matches, uv_init, uv_curr, xyzPoint = associate_by_triangulation(
                    new_matches,
                    cam,
                    other_MKF,
                    this_MKF,
                    {
                        ...
                        "min_reproj_err": self.settings["min_reproj_err"],
                    },
                )
                ...
                mappoint_indices = np.array(
                    [
                        ...
                        if self.data_manager.data.currentMap.mappoints[
                            i
                        ].most_recent_frame
                        >= this_MKF.frame_idx - n
                    ]
                )
                relevant_map_points = np.array(
                    [
                        self.data_manager.data.currentMap.mappoints[i].cartesian
                        for i in mappoint_indices
                    ]
                )

                # Filter for mappoints that are visible in this frame using estimated pose:
                relevant_uvs, valid = self.data_manager.config.cameras[
                    cam
                ].cartesian2image(
                    relevant_map_points, this_MKF.T_origin2body_estimated, padding=40
                ...
                for i in range(len(xyzPoint)):
                    ...
                    if len(inview_mappoint_indices) > 0:
                        ...
                        reproj_err = np.linalg.norm(
                            inview_relevant_uvs - uv_curr[i], axis=1
                        )
                        filter_reproj_err = reproj_err < 20

                        # Filter for if points are within 20m of eachother
                        filter_proximity = np.all(
                            np.abs(inview_relevant_map_points - xyzPoint[i])
                            < np.array([20, 20, 20]),
                            axis=1,
                        )

                        fuse_candidates = filter_reproj_err | filter_proximity
                        candidate_mappoint_indices = inview_mappoint_indices[
                            fuse_candidates
                        ]
                        candidate_descs = [
                            self.data_manager.data.currentMap.mappoints[i]
                            .observations[-1]
                            .descriptor
                            for i in candidate_mappoint_indices
                        ]
                        ...
                        if len(candidate_mappoint_indices) > 0:
                            desc_dist = np.linalg.norm(
                                candidate_descs
                                - this_MKF.descriptors[cam][new_matches[i].trainIdx],
                                axis=1,
                            )
                            index_min = min(
                                range(len(desc_dist)), key=desc_dist.__getitem__
                            )

                            min_mappoint = self.data_manager.data.currentMap.mappoints[
                                candidate_mappoint_indices[index_min]
                            ]
                            uvs, _ = self.data_manager.config.cameras[
                                cam
                            ].cartesian2image(
                                [min_mappoint.cartesian, [0, 0, 0]],
                                this_MKF.T_origin2body,
                            )
                            ...
                            uvs, _ = self.data_manager.config.cameras[
                                ...
                                [xyzPoint[i], [0, 0, 0]], this_MKF.T_origin2body
                            )
                            cv2.circle(
                                debug_img,
                                (int(uvs[0][0]), int(uvs[0][1])),
                                10,
                                [0, 255, 255],
                                thickness=5,
                                lineType=8,
                            ...
                            covariance = np.diag(
                                [10000, 10000, 10000]
                            )  # Assume high covariance 100m sigma, gtsam will figure out the covariances, no need to calculate jacobians here
                            ...
                            ptMap.addObservation(
                                ...
                                other_MKF.frame_idx,
                                uv_init[i],
                                other_MKF.descriptors[cam][new_matches[i].queryIdx],
                            )
                            ptMap.addObservation(
                                cam,
                                this_MKF.frame_idx,
